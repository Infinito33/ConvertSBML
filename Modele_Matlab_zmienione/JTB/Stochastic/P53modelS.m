...
function dy=P53modelS(t,y,NB,Amdm2,Apten,te,DNASw,ExtSw)
...
dy(3)=t0*y(9)-a4*y(3)*y(2)+c2*y(4)-(d0+d1*(NB^2/(NB^2+h0^2)))*y(3);         % cytoplasmic MDM2
dy(4)=a4*y(3)*y(2)-c2*y(4)-i0*y(4)-(d0+d1*(NB^2/(NB^2+h0^2)))*y(4)+e0*y(6); % cytoplasmic phospho-MDM2
dy(5)=t1*y(10)-d2*y(5);                                                     % cytoplasmic PTEN
dy(6)=i0*y(4)-(d0+d1*(NB^2/(NB^2+h0^2)))*y(6)-e0*y(6);                      % nuclear phospho-MDM2
dy(7)=p0-(a0+a1*(NB^2/(NB^2+h0^2)))*y(7)+c3*y(8)-(d3+d4*y(6)^n0)*y(7);      % nuclear P53
dy(8)=(a0+a1*(NB^2/(NB^2+h0^2)))*y(7)-c3*y(8)-(d5+d6*y(6)^n0)*y(8);         % nuclear phospho-P53 (P53p)
dy(9)=s0*Amdm2-d7*y(9);                                                     % MDM2 transcript
dy(10)=s1*Apten-d8*y(10);                                                   % PTEN transcript
dy(11)=p1*PPAA-d9*y(11);                                                    % Apoptotic factors
